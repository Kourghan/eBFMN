...
- (instancetype)initWith:(NSArray *)array groupedBy:(NSString *)group cellClass:(Class)cellClass reversed:(BOOL)reversed {
    ACCellClassBlock cellClassBlock = ^Class(NSIndexPath *indexPath, id object) {
        return cellClass;
    };
    return [self initWith:array groupedBy:group reversed:reversed cellClassBlock:cellClassBlock];
}

- (instancetype)initWith:(NSArray *)array groupedBy:(NSString *)group reversed:(BOOL)reversed cellClassBlock:(ACCellClassBlock)cellClassBlock {
    if (self = [super init]) {
        _cellClassBlock = cellClassBlock;
        _groupKey = group;
        _array = [array mutableCopy];
        _reversedGrouping = reversed;
...
- (Class<ACTableViewHeaderFooter>)classForFooterInSection:(NSInteger)section {
    return _footerClass;
}
